...
- **현황**: 좌(관심종목·섹터)/중(탭 전환 화면)/우(펀더멘털·호가·옵션) 3패널. 각 패널
  내부 스크롤 + 반응형(모바일 세로 스택)으로 화면이 잘리지 않는다. **좌/중/우 패널 경계를
  드래그해 폭을 조절**할 수 있고, 조절한 레이아웃은 localStorage에 자동 저장된다(새로고침/
  재접속 후 유지). 푸터의 "레이아웃 초기화"로 기본값 복원.
  (구현: `src/components/terminal/use-panel-layout.ts`)
- **예정(후속 슬라이스)**: 패널 드래그앤드롭 재배치, 개별 박스 높이 리사이즈,
  레이아웃 DB(`Setting`) 저장(다기기 동기화).
...
**부분 구현**: 레이아웃 — 좌/중/우 폭 드래그 조절 + localStorage 저장 완료(패널 DnD 재배치는 예정).

**보류(후속)**: 브로커 API 연동, 패널 드래그앤드롭 재배치, 한국주식+DART,
