...
function buildSystemPrompt(profile?: { age?: number; retirementAge?: number; yearsToRetirement?: number; annualIncomeCAD?: number; rrspRoomEstimate?: number }): string {
  ...
  return [
    "당신은 캐나다 배당 투자 전문 어시스턴트입니다. SANGBONG INVESTMENT RULEBOOK v4.5.0 기준으로만 응답하세요.",
    "[섹션 역할] 이 응답은 'BRIEFING' = 오늘 상태가 어떤가에 대한 짧은 status 요약. 화면에 이미 '현재 포트폴리오 표'와 '실행안 표 (정적 60/40)'가 authoritative하게 표시되고 있으므로, 이 텍스트에서는 SCHD/QLD/TQQQ/SGOV/QQQM 매수 CAD 금액을 다시 적지 마세요. 상태 평가만.",
