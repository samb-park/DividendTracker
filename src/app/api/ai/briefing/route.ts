...
function buildSystemPrompt(profile?: { age?: number; retirementAge?: number; yearsToRetirement?: number; annualIncomeCAD?: number; rrspRoomEstimate?: number }): string {
  ...
  const profileNote = notes.length > 0 ? ` 투자자 정보: ${notes.join(" / ")}. 은퇴 시점과 소득 수준에 맞는 TFSA/RRSP 전략을 우선시할 것.` : "";
  return `당신은 캐나다 배당 투자 전문 어시스턴트입니다. TFSA 연 $7,000 납입한도, RRSP 소득공제, US배당→TFSA 15% 원천징수 손실, RRSP는 면제.${profileNote} 투자자 프로필(나이, 목표)이 있으면 그에 맞는 맞춤 조언을 포함하세요. 최신 뉴스가 있으면 포트폴리오에 미치는 영향을 언급하세요. 답변은 3-5개 핵심 인사이트를 한국어로, 각 2문장 이내로 간결하게.`;
}
