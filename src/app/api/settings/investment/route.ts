...
// v4.5.0 rulebook defaults exposed to the Settings UI so it can pre-fill /
...
const RULEBOOK_NON_CORE_DEFAULTS = {
  ...
  SGOV: {
    frequency: "weekly" as const,
    cad: 0,                                  // v4.5.0: no rulebook-default refill
    rulebookVersion: "v4.5.0",
  },
};
...
const triggerParamsSchema = z.object({
  ...
  // upperTriggerPct removed: rulebook v4.5.0 removes the QLD emergency cap
...
export async function GET() {
  ...
  // we ignore it here (rulebook v4.5.0 removes the cap).
  ...
  return NextResponse.json({
    ...
    // v4.5.0: rulebook-default non-core CAD streams. UI uses these as fallback
